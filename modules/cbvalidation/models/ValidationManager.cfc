...
component accessors="true" serialize="false" implements="IValidationManager" singleton{
	...
	*/
	ValidationManager function init( struct sharedConstraints=structNew() ){
		...
		variables.validValidators &= ",optionalInList,optionalUdf";
	...
	*/
	cbvalidation.models.validators.IValidator function getValidator(required string validatorType, required any validationData){
		switch( arguments.validatorType ){
			...
			// custom Validator that returns true for NULLs and empty string
			case "optionalInList"	: { return wirebox.getInstance("cbvalidation.models.validators.OptionalInListValidator"); }
			case "optionalUdf"		: { return wirebox.getInstance("cbvalidation.models.validators.OptionalUDFValidator"); }
